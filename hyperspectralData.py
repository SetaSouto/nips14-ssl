...
class HyperspectralData:
    ...
    def load_numpy(self, n_train, n_valid=10000, n_test=10000, unlabeleds=True, mineralogy=True, file="ALH1599-17-labeled.hdf5"):
        ...
        :param n_vaild: Numbers of examples for validation.
        ...
        pixels = pixels/(pixels.max()+1)

        #Now, select randomnly the datasets

        maxRow = pixels.shape[1]
        totalToBeSelected = n_train + n_test + n_valid

        #Generate a random narray with the numbre of rows to be selected without repetition.
        rows = np.random.choice(maxRow, size=totalToBeSelected, replace=False)

        valid_rows = rows[0 : n_valid]
        test_rows = rows[n_valid : (n_valid+n_test)]
        train_rows = rows[(n_valid+n_test) : totalToBeSelected]
        ...
        label1 = labels[rows[n_valid]] # The correspondant label
        data2 = test_x[:, 0] # First sample of test dataset.
        label2 = test_y[0] # The correspondant label.
        if (data1 != data2).all():
